...
function buildGroups(source: ExtensionSummary[]): CategoryGroup {
  return source.reduce<CategoryGroup>((groups, extension) => {
    ...
    const href = getExtensionUrl({
      slug: extension.slug,
      id: extension.id,
      developerUsername: extension.developerUsername
    });
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-8">
            ...
            <Link
              href="/best/ad-blocker"
              className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-colors"
            >
              Ad Blocker
            </Link>
            <Link
              href="/best/grammar-checker"
              className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-colors"
            >
              Grammar Checker
            </Link>
            <Link
              href="/best/screen-recorder"
              className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-colors"
            >
              Screen Recorder
            </Link>
            <Link
              href="/best/pdf-editor"
              className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-colors"
            >
              PDF Editor
            </Link>

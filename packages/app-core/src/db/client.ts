...
function isDbDisabled(): boolean {
  const flag = process.env.SERP_EXTENSIONS_DISABLE_DB || process.env.DISABLE_DB;
...
function resolveConnectionString(): string | null {
  const url = process.env.DATABASE_URL ?? process.env.SERP_EXTENSIONS_DB_DATABASE_URL ?? process.env.POSTGRES_URL;
